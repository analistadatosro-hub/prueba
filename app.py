...
def login_page():
    ...
    with col2:
        ...
        ">
            <h1 style="color:white">SODE<span style="color:#EF4044">X</span>O PERÚ</h1>
            <h3 style="color:#9fbad6;font-weight:normal">Acceso Rutograma</h3>
        </div>
        """, unsafe_allow_html=True)
...
# --- STAGE 2: CONFIGURACION DE FLOTA ---
    ...
    with col_config:
        ...
        # Assuming huge capacity if not specified, implicitly 1 vehicle = 1 route logic basically
        # The user said "Cada vehiculo es 1 ruta" which logic implies.
        
        max_capacity = st.number_input("Capacidad Max (Tickets por camioneta)", min_value=1, value=100)
        
        col_adv_1, col_adv_2 = st.columns(2)
        service_time = col_adv_1.number_input("Tiempo de Servicio por Ticket (min)", min_value=1, value=15, help="Tiempo estimado para entregar 1 ticket")
        max_work_hours = col_adv_2.number_input("Jornada Maxima (horas)", min_value=4, value=12, help="Duración máxima de ruta por camioneta")
        ...
        if col_act_2.button("🚀 Calcular Rutas", type="primary"):
            # Prepare Data - AGGREGATE BY OFFICE to ensure 1 visit per office
            df_raw = pd.DataFrame(st.session_state.daily_tickets)
            
            # Group by Name/Location
            # We aggregate: 'Importe de la entrega' (sum), and keep the first Lat/Lon found
            # Group by Name/Location/Client to preserve Client info
            df_grouped = df_raw.groupby(['Nombre', 'Latitud (y)', 'Longitud (x)', 'Habla a']).agg({
                'Importe de la entrega': 'sum',
                'Ticket': lambda x: ', '.join(x.astype(str)), # Join ticket IDs
            ...
            with st.spinner("Optimizando rutas..."):
                ...
                if solution:
                    ...
                    est_travel_mins = num_offices * 30 * tf_factor # Rough est 30 mins per stop
                    total_work_load_mins = total_service_mins + est_travel_mins
                    
                    # 1. Check Single Node Limits
                    ...
                    if max_single_time_mins > work_day_mins:
                        ...
                        **Diagnóstico del Fallo:**
                        El sistema no encontró solución. Revise los siguientes límites:
                        ...
                        **Sugerencia:**
                        ...
                        Si ya tiene {suggested_min}, el problema puede ser la dispersión geográfica (tiempos de viaje muy largos entre puntos). Intente aumentar ligeramente la jornada laboral (+1h).
                        """)
...
# --- STAGE 3: RESULTADOS ---
    ...
    st.dataframe(style_dataframe(df_res[['VehicleID', 'OrderInRoute', 'LocationName', 'Client', 'AccumulatedDuration_Mins']]), use_container_width=True)
    ...
    for i, vid in enumerate(unique_vehicles):
        ...
        with st.expander(f"Camioneta #{vid + 1} ({len(vehicle_route)} paradas)", expanded=True):
            ...
            for _, row in vehicle_route.iterrows():
                ...
                # Construct coordinate string for Maps
            ...
            if maps_waypoints:
                ...
                # We simply list all points. Google Maps will treat the first as origin, last as dest, and others as waypoints.
                # Or we can prefix with 'Current+Location' if the driver starts from their position.
                # Let's use the points directly so the route is fixed as planned.
            ...
            for _, stop in vehicle_route.iterrows():
                ...
                # Try to find ticket info matches
                # This is a bit tricky since df_res is aggregated from the solver result
                # But we can cross reference st.session_state.daily_tickets if needed
                # For now, just the location and order is what was asked.
